...
export default function DeptCatalogProgramsPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h3 className="text-base font-display font-bold text-slate-800">Courses</h3>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {PROGRAM_TYPES.map(pt => (
          <button
            key={pt.id}
            onClick={() => navigate(`/departments/${deptId}/academics/courses/${pt.id}`)}
          ...
          >
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${pt.color.bg}`}>
              <GraduationCap size={22} className={pt.color.icon} />
            </div>
