...
import { adminProgramService, adminDeptService, courseMaterialService } from '@/app-modules/departments/api/adminCoursesApi'
...
export default function DeptLMSemestersPage() {
  ...
  const base      = `/departments/${deptId}/academics/materials/${programId}`

  return (
    <div className="space-y-5">
      <nav className="flex items-center gap-1 text-sm">
        <button onClick={() => navigate(`/departments/${deptId}/academics/materials`)}
          className="text-slate-500 hover:text-brand-600 transition-colors">Materials</button>
        <ChevronRight size={13} className="text-slate-300" />
        <span className="text-slate-700 font-medium">{program?.name ?? programId}</span>
      </nav>

      <div>
        <h3 className="text-base font-display font-bold text-slate-800">{program?.name} — Semesters</h3>
        <p className="text-sm text-slate-500">{program?.fullName} · {deptShortName} · {program?.maxSemesters} semesters</p>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {semesters.map(sem => {
          const count = catalogDept
            ? courseMaterialService.countForSemester(programId!, catalogDept.id, sem)
            : 0
          return (
            <button key={sem} onClick={() => navigate(`${base}/${sem}`)}
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  {count > 0 ? `${count} material${count !== 1 ? 's' : ''}` : 'No materials yet'}
                </p>
              </div>
            </button>
          )
