...
import { adminProgramService, adminDeptService, timetableSectionService } from '@/app-modules/departments/api/adminCoursesApi'
...
export default function DeptTTSemestersPage() {
  ...
  const base      = `/departments/${deptId}/academics/timetable/${programId}`
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {semesters.map(sem => {
          const count = catalogDept
            ? timetableSectionService.countForSemester(programId!, catalogDept.id, sem)
            : 0
          return (
            <button key={sem} onClick={() => navigate(`${base}/${sem}`)}
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  {count > 0 ? `${count} section${count !== 1 ? 's' : ''}` : 'No sections yet'}
                </p>
              </div>
            </button>
          )
